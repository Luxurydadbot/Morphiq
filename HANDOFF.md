...
**Still open / not yet done:** the real logo files have NOT been placed into the repo yet — `public/logo192.png` / `logo512.png` are still the default React placeholder icon. Next step when picked back up: generate proper 192x192/512x512 (and iOS/Android app-icon-size) PNGs from the new vector mark and replace the placeholder files, then re-verify the PWA manifest still points at them correctly.
