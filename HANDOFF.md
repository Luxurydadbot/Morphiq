...
**Still open / not yet done:** (1) the icon mark itself is not finalized -- three directions were sketched (faceted H monogram, orbit, neural hub) and shown paired with the wordmark as lockups, no final pick yet. (2) Once an icon is picked, `public/logo192.png`/`logo512.png` (still the default React placeholder) need to be regenerated from the real vector mark and swapped in, then the PWA manifest re-verified.
